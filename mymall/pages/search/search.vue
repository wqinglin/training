	...
	<view class="app column">
		<view class="search-wrap center">
			<view class="input-box row">
				...
				<!-- @confirm="search"  回车 调用 search ， @click="search" 点击按钮 search-->
				<input 
					class="input"
					type="text" 
		...
		<view class="content">
			...
			<view v-if="hotList.length > 0" class="list">
				<view class="item center" v-for="(item, index) in hotList" :key="index" @click="search(item)">
					<text>{{item}}</text>
				</view>
			</view>
		</view>
