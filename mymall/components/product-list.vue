<template>
	<view class="mix-product-list" :class="{'is-row': listType === 'row'}">
		...
		>
			...
			<view class="right column">
				...
				<view class="price-wrap row">
					<!-- <mix-price-view :price="item.ori_price" :size="34"></mix-price-view> -->
					<text v-if="item.price > item.ori_price" class="m-price">￥{{ item.price }}</text>
					<view class="tags row">
		...
		>
			...
			<view class="price-wrap row">
				<!-- <mix-price-view :price="item.ori_price"></mix-price-view> -->
				<text v-if="item.price > item.ori_price" class="m-price">{{ item.price }}</text>
...
<style scoped lang="scss">
	/* .lazypic{
		background: url(../static/empty/cart.png);
	}
	.loaded{
		background:none;
	} */
